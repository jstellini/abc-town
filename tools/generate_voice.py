...
async def synth_one(sem, key, text, voice, rate, pitch):
    async with sem:
...
async def main_async(args):
    ...
    sem = asyncio.Semaphore(8)  # be polite to the (free) TTS endpoint
    ...
    manifest_lines = [f'  "{key}": "assets/voice/{key}.mp3",' for key, _ in lines]
    manifest = (
    ...
    print(f"\nWrote {len(todo)} clips to {VOICE_DIR}")
    print(f"Wrote manifest to {MANIFEST_JS}")
