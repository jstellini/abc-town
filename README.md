...
- **Reveal** – the new friend arrives as a **silhouette** filling the screen, the same shadow the
  intro teased: *"Who's inside? Tap to see!"* **Six taps** drive the shadow back. It shrinks
  inwards from the feet and hands, closing in on the face — the last thing you see is who it is —
  until the sixth tap lifts it: colour, confetti, fanfare, the letter and the name, *"A is for
  Apple!"* Every tap sparkles where the finger lands and rings a rung higher. A child who doesn't
  tap is never stuck: it nudges after seven seconds and opens itself after fourteen, counted from
  the last tap. Tapping the character after that giggles, as before.
...
- **The reveal** – `REVEAL_TAPS` and `REVEAL_RADII` in `js/app.js`: how many taps it takes and how
  much shadow is left after each one (one entry per tap, the last tap clearing it). A `circle()`
  percentage resolves against the diagonal of the art, so the numbers look smaller than they are:
  it takes about 62% to cover a whole character, so the first tap has to drop well inside that or
  nothing appears to happen, and 18% is a face. The shadow is centred on the friend's actual face,
  read at runtime from the `class="face"` pivot that `build_characters.py` writes into each SVG
  (`learnFace`, prefetched from the intro screen) — Xylophone wears its face below the middle and
  Moon's is off to one side, and a table here would drift from the art. The nudge and the give-up
  timers are `armReveal`, and `Sfx.rise` climbs one rung per tap.
